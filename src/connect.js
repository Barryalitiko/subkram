...
async function connect(subbotNombre = null) {
  ...
  const socket = makeWASocket({
    ...
    getMessage,
    subbot: subbotNombre ? true : false
  });
  ...
  if (!socket.authState.creds.registered) {
    ...
    if (!phoneNumber) {
      errorLog('Numero de telefono inválido! Reinicia con el comando "npm start".');
      process.exit(1);
    }
  ...
  socket.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      if (statusCode === DisconnectReason.loggedOut) {
        ...
        const newSocket = await connect(subbotNombre);
        load(newSocket);
      }
