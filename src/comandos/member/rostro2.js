...
module.exports = {
  ...
  handle: async ({ socket, remoteJid }) => {
    try {
      ...
      // Si el usuario tiene tortuga o love, agregar con ffmpeg
      const webpPath = path.resolve(tempPath, `personaje_${remoteJid}.webp`);
      if (usuarios[remoteJid].objetos.includes("tortuga") || usuarios[remoteJid].objetos.includes("love")) {
        let filters = "";
        if (usuarios[remoteJid].objetos.includes("tortuga")) {
          filters += `[1:v]scale=75:144[tortuga];[0:v][tortuga]overlay=153:162[t];`;
        }
        if (usuarios[remoteJid].objetos.includes("love")) {
          filters += `[2:v]scale=50:50[love];[t][love]overlay=100:100`;
        }

        const ffmpegCommand = `ffmpeg -i "${pngPath}" ${usuarios[remoteJid].objetos.includes("tortuga") ? `-i "${tortugaPath}"` : ""} ${usuarios[remoteJid].objetos.includes("love") ? `-i "${lovePath}"` : ""} -filter_complex "${filters}" -loop 0 -y "${webpPath}"`;
