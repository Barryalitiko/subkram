...
const downloadMusic = (url) => {
  return new Promise((resolve, reject) => {
    const outputDir = path.join(__dirname, "downloads", "music");
    ensureDirectoryExists(outputDir); // Asegura que la carpeta para música exista

    const outputFile = path.join(outputDir, `${Date.now()}.mp3`);
...
const downloadVideo = (url) => {
  return new Promise((resolve, reject) => {
    const outputDir = path.join(__dirname, "downloads", "videos");
    ensureDirectoryExists(outputDir); // Asegura que la carpeta para videos exista

    const outputFile = path.join(outputDir, `${Date.now()}.mp4`);
